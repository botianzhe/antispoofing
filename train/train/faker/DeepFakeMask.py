...
class components(Mask):  # pylint: disable=invalid-name
    ...
    def build_mask(self):
        ...
        nose_ridge = (self.landmarks[19:25], self.landmarks[27:28])
        r_eye = (self.landmarks[36:42],
                 self.landmarks[27:28])
        l_eye = (self.landmarks[42:48],
                 self.landmarks[27:28])
        nose = (self.landmarks[27:31], self.landmarks[31:36])
        mouse=(self.landmarks[48:55], self.landmarks[55:68])
        parts = [r_jaw, l_jaw,nose_ridge, r_eye, l_eye, nose,mouse]
        randlen=random.randint(3,5)
        selectparts=random.sample(parts,randlen)
        # for r in parts:
        for item in selectparts:
            merged = np.concatenate(item)
            cv2.fillConvexPoly(mask, cv2.convexHull(merged), 255.)  # pylint: disable=no-member
        return mask
